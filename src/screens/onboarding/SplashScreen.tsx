  ...
  return (
    <SafeAreaView style={[s.container, { backgroundColor: theme.bg }]}>
      ...
      <View style={s.authBlock}>
        <TouchableOpacity
          style={[s.btn, { backgroundColor: theme.fg }]}
          onPress={() => nav.navigate('OnboardingBasics')}
        >
          <Svg width={16} height={16} viewBox="0 0 16 16">
            <Path d="M12.5 8.5c0-2.5 2-3.6 2.1-3.7-1.1-1.7-2.9-1.9-3.5-1.9-1.5-.2-2.9.9-3.7.9-.8 0-2-.9-3.2-.8-1.6 0-3.1.9-4 2.4-1.7 3-.4 7.4 1.2 9.8.8 1.2 1.8 2.5 3.1 2.4 1.2-.1 1.7-.8 3.2-.8s1.9.8 3.2.7c1.3 0 2.2-1.2 3-2.4.9-1.3 1.3-2.7 1.3-2.8-.1 0-2.7-1-2.7-3.8z" fill={theme.bg}/>
          </Svg>
          <Text style={[s.btnTxt, { color: theme.bg, fontFamily: FONTS.bold }]}>Continuar con Apple</Text>
        </TouchableOpacity>
        <TouchableOpacity
          style={[s.btn, { backgroundColor: theme.surface2, borderColor: theme.line2, borderWidth: 1 }]}
          onPress={() => nav.navigate('OnboardingBasics')}
        >
          <Text style={[s.btnTxt, { color: theme.fg, fontFamily: FONTS.bold }]}>Continuar con Google</Text>
        </TouchableOpacity>
        <TouchableOpacity
          style={[s.btnGhost, { borderColor: theme.line2 }]}
          onPress={() => nav.navigate('OnboardingBasics')}
        >
          <Text style={[s.btnTxt, { color: theme.fg2, fontFamily: FONTS.medium }]}>Continuar con email</Text>
        </TouchableOpacity>

        {/* Dev shortcut: skip onboarding */}
        <TouchableOpacity
          style={[s.btnSkip, { borderColor: theme.accent }]}
          onPress={() => completeOnboarding()}
        >
          <Text style={[s.btnTxt, { color: theme.accent, fontFamily: FONTS.mono }]}>EXPLORAR APP →</Text>
        </TouchableOpacity>
...
const s = StyleSheet.create({
  ...
  btnSkip: { height: 44, borderRadius: 99, borderWidth: 1, alignItems: 'center', justifyContent: 'center', marginTop: 4 },
